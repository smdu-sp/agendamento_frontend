...
export function ArthurSaboyaServiceCard({
  ...
  return (
    <Link href={href} className="group block h-full w-full md:max-w-[496px]">
      ...
      >
        <div className="flex min-h-0 flex-1 flex-col gap-4 sm:flex-row sm:items-start sm:gap-6">
          <div
          ...
          <div className="min-w-0 flex-1">
            ...
            <ul className="flex flex-col gap-2 bg-white p-0 md:min-h-[104px] md:max-w-[293px]">
              {features.map((feature, index) => (
                <li
                  key={index}
                  className={`${openSans.className} flex items-start gap-2.5 text-[13px] leading-snug text-[#4C575F] sm:gap-3 sm:text-[14px] sm:leading-[1.35]`}
                >
